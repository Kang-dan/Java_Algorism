...
public class java_SWEA_4789 {
	...
	public static void main(String[] args) {
		...
		for(int test_case=1; test_case<=T; test_case++) {
			...
			int newPerson = 0; //고용해야할 사람수 
			int sumPerson = person.charAt(0)-'0'; //기립박수 치는 사람수 
			for(int i=1; i<person.length(); i++) {
				if(person.charAt(i) == '0') continue; // 해당하는 조건에 사람이 없는 경우 
				if(sumPerson >= i) {
					sumPerson += person.charAt(i)-'0';
					
				}else if(sumPerson < i){
					newPerson += (i-sumPerson); //새로 고용해야할 사람
					sumPerson += (i - sumPerson) + (person.charAt(i)-'0'); //(i-sumPerson)임! 여기 로직 생각 잘하기.
				}
			}
