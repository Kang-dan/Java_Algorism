...
public class java_SWEA_5356 {

	public static void main(String[] args) {
		/* StringBuilder()와 append()사용 */
		Scanner sc = new Scanner(System.in);
		int T = sc.nextInt(); //테스트 케이스 
		
		for(int test_case=1; test_case<=T; test_case++) {
			String[] arr = new String[5]; //5개의 문자열 입력받기 위해서
			
			//문자열 입력받기 
			int max_length = 0; //최대 긴 문자열 
			for(int i=0; i<5; i++) {
				arr[i] = sc.next();
				if(max_length < arr[i].length()) max_length = arr[i].length();
			}
			
			//세로로 출력하기 (비어있는 부분 출력 안하게 조심)
			StringBuilder result = new StringBuilder();
			for(int i=0; i<max_length; i++) {
				for(int j=0; j<5; j++) {
					 if(i < arr[j].length())//비어있는 문자열 출력하면 안되니까
						result.append(arr[j].charAt(i)); //append() 이어붙여줌
				}
			}
		...
		for(int test_case=1; test_case<=T; test_case++) {
			...
			char[][] copy = new char[5][15];
			for(int i=0; i<5;i++) {
				for(int j=0; j<15; j++) {
					copy[i][j] = '가'; //배열 초기화 
				}
			}
			int max_length = 0; //가장 긴 문자열 
			for(int i=0; i<5; i++) {
				arr[i] = sc.next();
				if(max_length < arr[i].length()) max_length = arr[i].length();
				for(int j=0; j<arr[i].length(); j++) {
					copy[i][j] = arr[i].charAt(j); //String ->char배열로 
				}
			}
			...
			for(int i=0; i<max_length; i++) {
				for(int j=0; j<5; j++) {
					if(copy[j][i] == '가') continue;
					else System.out.printf("%c", copy[j][i]);
				}
			}
			System.out.println();
